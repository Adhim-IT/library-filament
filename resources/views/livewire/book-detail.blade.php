<div class="w-full max-w-[85rem] py-40 px-4 sm:px-6 lg:px-8 mx-auto">
    <div class="bg-gray-950 p-8 rounded-lg shadow-lg">
        <div class="flex flex-col items-center lg:items-start">
            ...
            @if ($isBorrowed)
                <a href="{{ route('return-book', ['borrowId' => $borrow->id]) }}" class="bg-amber-400 text-gray-200 px-6 py-2 rounded-lg font-semibold hover:bg-amber-500 transition duration-300 ml-80 mt-6">
                    Kembalikan
                </a>
            @else
                <button wire:click="borrowBook({{ $book->id }})" class="bg-amber-400 text-gray-200 px-6 py-2 rounded-lg font-semibold hover:bg-amber-500 transition duration-300 ml-80 mt-6">
                    Pinjam
                </button>
            @endif
        </div>
